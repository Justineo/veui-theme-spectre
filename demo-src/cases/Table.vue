<template>
  <article>
    ...
    <section>
      <veui-checkbox-group ui="inline" type="checkbox" v-model="columns" :items="[
      ...
      <veui-table ui="bordered" :data="data" :column-filter="columns" :key-field="selectSpanRow ? 'group' : 'id'" selectable
        ...
        <veui-table-column field="updateDate" title="更新时间" align="center">
          <template slot-scope="props">
            <span :ref="`time-a-${props.item.id}`">{{ props.item.updateDate }}</span>
            <veui-tooltip :target="`time-a-${props.item.id}`">{{ props.item.updateDate }}</veui-tooltip>
          </template>
        </veui-table-column>
    ...
    <section class="container">
      <veui-table ui="hover striped" :data="data" :column-filter="columns" key-field="id"
        selectable select-mode="single" :selected.sync="selected2">
        <veui-table-column field="id" title="数据 ID"/>
        <veui-table-column v-if="showGroup" field="group" title="数据分组">
        </veui-table-column>
        <veui-table-column field="desc" title="数据描述"/>
        ...
        <veui-table-column field="updateDate" title="更新时间" align="right">
          <template slot-scope="props">
            <span :ref="`time-b-${props.item.id}`">{{ props.item.updateDate }}</span>
            <veui-tooltip :target="`time-b-${props.item.id}`">{{ props.item.updateDate }}</veui-tooltip>
          </template>
        </veui-table-column>
  ...
  methods: {
    ...
    append () {
      let item = {
        ...
        updateDate: Date.now() + Math.floor(Math.random() * 1e10)
      }
      this.nextId++
      this.nextIndex++
      this.data.push(item)
